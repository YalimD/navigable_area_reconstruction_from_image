...
def extractCameraParameters(image, warped_img, model_points, image_points, K_warped, H = None):
    ...
    _ret, rvec_top, tvec_top = cv2.solvePnP(model_points, image_points, K_warped, dist_coef, flags=cv2.SOLVEPNP_ITERATIVE)
    ...
    #Intrinsic Line
    camWriter.write("{} {} {} {} {} {}\n".format(w_warped, h_warped, K_warped[0][2], K_warped[1][2], K_warped[0][0], K_warped[1][1]))
...
def rectify_groundPlane(image_path, segmented_img_path, detection_data_file):
    ...
    for i, k in enumerate(vp_determination_methods):
        ...
        K_warped = np.array([[focal_length, 0, warped_result.shape[1]/2],
                     [0,focal_length, warped_result.shape[0]/2],
                     [0,0,1]])

        #Output the internal and external parameters through a text file
        extractCameraParameters(image, warped_result,  model_points, image_points, K_warped, H)
